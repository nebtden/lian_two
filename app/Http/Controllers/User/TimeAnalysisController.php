...
use App\Models\UsersTimeAnalyses;
...
class TimeAnalysisController extends Controller
{
    ...
    public function index(){
        ...
        $list =  UsersTimeAnalyses::where(
            $where
        )->orderBy('id','desc')->paginate(7);

        $total = UsersTimeAnalyses::where(
            $where
        )->count();
            $status = UsersTimeAnalyses::$status;



        return view('time_analysis.index',[
            'list'=>$list,
            'status'=>$status,
            'total'=>$total,
        ]);
